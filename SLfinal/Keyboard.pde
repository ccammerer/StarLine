void keyPressed() {
  ...
  if (pantalla == PANTALLA.HOME && (next.mouseOverButton()||prev.mouseOverButton())) {
    if (keyCode==LEFT && pc.numPage >0) {
      pc.numPage = pc.numPage-1;
      println("NUM PAGE: ");
    }
    // Anar un mes endavant
    else if (keyCode==RIGHT&& pc.numPage <=4) {
      pc.numPage = pc.numPage+1;
      println("NUM PAGE: ");
    }
  }
