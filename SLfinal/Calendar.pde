...
class Calendario {
  ...
  // Calendario actual, i del mes anterior
  Calendar cal, cPrev;
  
  // Botons del Calendario
  DayButton[] buttons;
  
  // Dimensions del Calendario
  int x, y, w, h;
  ...
  // Constructor
  Calendario(int x, int y, int w, int h, String[][] datesClau){
  ...
  // Assigna dies especials a mostrar en el Calendario
  ...
  // Va un mes enrera en el Calendario
  void prevMonth(){
  ...
  // Va un mes endavant en el Calendario
  void nextMonth(){
  ...
  void display(){
    ...
    if(dateSelected){
      ...
      fill(0); textSize(20); textAlign(RIGHT);
      text(dateText, x+w, y - 5);
    }
  }
  
  
  // Comprova si pitjam sobre els botons del Calendario
  void checkButtons(){
    for(DayButton b : buttons){
  ...
  // Deselecciona tots els botons del Calendario
  void deselectAll(){
    for(DayButton b : buttons){
      if(b!=null){
        b.setSelected(false);
